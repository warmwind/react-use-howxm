...
export interface IUseHowxm {
  readyState: boolean
  initHowxm: (appId: string, logCallback?: (...data: unknown[]) => void) => boolean
  identifyHowxm: (customerInfo: TCustomerInfo, logCallback?: (...data: unknown[]) => void) => boolean
  checkOpenHowxm: (
    campaignId: string,
    uid: string,
    onSuccess?: () => void,
  ...
  eventHowxm: (eventCode: string, eventAttrs?: TAttribute, logCallback?: (...data: unknown[]) => void) => void
  setExtraAttributes: (extraAttrs: TAttribute, logCallback?: (...data: unknown[]) => void) => void
}
